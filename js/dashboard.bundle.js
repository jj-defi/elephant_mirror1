...
async function bindUI() {
    ...
    let processingFee = parseInt(await routerProxy.methods.processingFee().call())
    let referralFee = parseInt(await routerProxy.methods.referralFee().call())


    let referrerAddress = await referralData.methods.myReferrer().call()
    hasReferrer = referrerAddress != referrerAddress == BANKROLL['BSC'].Zero

    referralFee = (hasReferrer) ? 0 : referralFee
    let precision = parseInt(await routerProxy.methods.precision().call())
    let feeMultiple = (precision - processingFee + referralFee) / precision
    let discount = (100 - processingFee + referralFee) / 10
    ...
    let buyTokens = async (e) => {
        try {
            ...
            amount = resolveInput(amount, 1e18)
            ...
            buyEstimate.html(`${formatSun(amount)} BNB ${approxStr} ${formatElephant(elephantAmount[1] * feeMultiple)} ELEPHANT with ${discount.toFixed(1)}% discount`)
        } catch (error) {
            console.error(error)
        }
    ...
    let sellTokens = async (e) => {
        try {
            ...
            amount = resolveInputAndBalance(amount, balance, 1e9)
            ...
            sellEstimate.html(`${formatElephant(amount)} ELEPHANT ${approxStr} ${formatSun(Math.floor(bnbAmount[1] * feeMultiple))} BNB with ${discount.toFixed(1)}% discount`)
        } catch (e) {
            console.error(e)
        }
    ...
    let transferTokens = async (e) => {
        try {
            ...
            amount = resolveInputAndBalance(amount, balance, 1e9)
            transferEstimate.html(`${formatElephant(amount)} ELEPHANT => ${formatElephant(0.97 * amount)} ELEPHANT`)
        } catch (e) {
            console.error(e)
        }
...
async function isStakeEnabled() {
    let allowance = await elephant.methods.allowance(currentAddress, routerProxy.options.address).call();
    return allowance > 0 ? true : false
}

async function enableStake() {
    let web3js = getWeb3()
    elephant.methods.approve(routerProxy.options.address, web3js.utils.toBN(MAX_INT_HEX)).send()
...
async function disableStake() {
    let web3js = getWeb3()
    elephant.methods.approve(routerProxy.options.address, web3js.utils.toBN(0)).send()
        .on('transactionHash', (tx) => refresh(tx))
        .on('error', (e) => txError(e))
    ...
    */
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        if (address.length == 0){
         ...
         else if (!web3js.utils.isAddress(address)) {
            $('#invalidAddressModal').modal()
            return false 
        } 

        
        await routerProxy.methods.swapExactETH(
            web3js.utils.toBN(0),
            address)
            .send({value: amount})
            .on('transactionHash', (tx) => refresh(tx))
...
async function transfer() {
    ...
    */
    ...
    if (!isStaking) {
        showAlert('Enable ELEPHANT', 'ELEPHANT is not enabled.  Look for the toggle and make sure it is on (blue)!')
        return
    }

    let address = $('#recipient').val().trim()
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        await routerProxy.methods.transferExactTokens(
            amount,
            address)
            .send()
            .on('transactionHash', (tx) => refresh(tx))
...
async function sell() {
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
       ...
       await routerProxy.methods.swapExactTokens(
            amount,
            web3js.utils.toBN(0),
            currentAddress)
            .send()
...
async function showStats() {
    try {
        ...
        let [liquidity, liquidity2, graveyardAmount, elephantTreasuryBalance, supply, ldriveBalance] =
            await Promise.all(
                [
                    ...
                    elephant.methods.balanceOf(ldrive.options.address).call()
                ]
            )
        ...
        $('#ldrive-balance').text(formatElephant(ldriveBalance))
...
async function showUserStats() {
    web3js = getWeb3()
    let [bnbBalance,balance, supply,graveyardAmount, referrerAddress] =
        await Promise.all(
            [
    ...
    let processingFee = await routerProxy.methods.processingFee().call()
    let precision = await routerProxy.methods.precision().call()
    let feeMultiple = processingFee / precision

    hasReferrer = referrerAddress != referrerAddress == BANKROLL['BSC'].Zero

    if (referrerAddress == BANKROLL['BSC'].Zero){
        $('#partner-referrer').html(`<a href="/trunk.html">set partner to share/gain 0.5% rewards</a>`)
    } else {
        $('#partner-referrer').text(`rewards 0.5% ${shortId(referrerAddress,5)} / 8% BERTHA`)
    }
    
    $('#current-raffle').html((participant['round'] == raffleStats['currentRound']) ? `you are entered in the raffle` : `to enter the Stampede Raffle <br> trade ${formatElephant(await toElephantPrice(raffleMin / feeMultiple))}+ ELEPHANT ${approxStr} ${formatSunX(raffleMin / feeMultiple, 0)} USD`)

    $('.staking-status').text(isStaking ? `ELEPHANT Enabled` : `ELEPHANT Disabled`)
    $('#stakingChb').prop('checked', isStaking)
    $('#transferChb').prop('checked', isStaking)
