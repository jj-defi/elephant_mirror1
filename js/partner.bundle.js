...
var balanceFeed = []
let collateral_price, first_block, contract_timestamp, symbol, padding, asset_symbol, refUrl
...
async function bindUI() {
    ...
    if (urlParams.has('ref')){
        ...
        $('#partner-referrer-input').val(urlParams.get('ref'))

    }

    refUrl = `${document.location.origin}/partner.html?ref=${currentAddress}`

    new QRCode(document.getElementById("qrcode"), refUrl)

    $('#current-long').text(currentAddress)

}

async function showQR() {
    $('#qrcode img').css('display', 'inline')
    $('#qrModal').modal()
...
async function showStats() {
    try {
        let web3js = getWeb3()
        let [stampedeStats, futureStats, totalAirdrops] =
            await Promise.all(
                [
                    stampede.methods.getInfo().call(),
                    futures.methods.getInfo().call(),
                    sponsorReader.methods.total_sponsored().call()      

                ]
            )
           
        $('#partner-total-airdrops').text(formatSunX(totalAirdrops, 2))
        $('#partner-total-partners').text(`${numeral(stampedeStats['total_users']).format('0,0.000 a').toUpperCase()} / ${numeral(futureStats['total_users']).format('0,0.000 a').toUpperCase()}`)
...
async function showUserStats() {
    ...
    $('#partner-pending').text(formatSunX(airdropStats[0], 4))
