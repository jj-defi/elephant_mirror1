...
async function main() {
    ...
    let tokens  //Let's keep tokens streamlined for the homepage
    if (window.ON_HOMEPAGE){
        tokens = ['trunk']
    } else {
        tokens = ['wbnb', 'bdollar', 'binance-bitcoin', 'busd', 'busdv2', 'wbnbv2','ethereum', 'binance-bitcoinv2', 'pancakeswap-token', 'trunk']  
    }
    ...
    for(let i=0; i < tokens.length; i++) {
        ...
        try {
            ...
            let [tokenBalance, totalSupply, dividendBalance, bdo_price] =
                await Promise.all(
                    [
                        stack.methods.totalTokenBalance().call(),
                        stack.methods.totalSupply().call(),
                        stack.methods.dividendBalance_().call(),
                        coingeckoPrice((token == 'trunk') ? 'busd' : token.split('v2')[0])
                    ]
                )

                if (token == 'trunk'){
                    trunkSupply = await trunk.methods.totalSupply().call()
                    local_tvl = parseInt(trunkSupply) * bdo_price
                } else {
                    local_tvl = parseInt(tokenBalance) * bdo_price
                }
                sumTVL += local_tvl
            ...
            let apy, apr, dailyEstimate

            if (token == 'trunk') {
                dailyEstimate = await rewardPool.methods.dailyEstimate().call() 
                apy = totalSupply ? 100 * dailyEstimate * 365 / totalSupply : 0   
                /*apr = totalSupply ? dailyEstimate * 365 / totalSupply : 0    
                ...
                apy = dripApy + instantApy
            }

            if (apy > 1e9){
                $(`#${token}-apy`).html(`${infiniteStr} % APR`)
            } else {
                if (token == 'trunk'){
                    $(`#${token}-apy`).text(`${numeral(apy).format('0.000 a').toUpperCase()}`)
                } else {
                    $(`#${token}-apy`).text(`${numeral(apy).format('0.000 a').toUpperCase()} % APR`)
                }
            }
            
            $(`#${token}-tvl`).text(`${formatSunX(local_tvl,1)} USD`)
    ...
    //ELEPHANT marketcap
    let supply = await elephant.methods.totalSupply().call()  
    let flowAPR = await flowEngine.methods.scaleByPeg(web3js.utils.toBN(205e18)).call()  
    let price = await getElephantPrice(1e9)
    let marketcap =   price * supply / 1e9  
    let treasury =  await elephant.methods.balanceOf(elephantTreasury.options.address).call()
    treasury = price * treasury / 1e9
    
    //TVL
    sumTVL = await tvl()

    $('#flow-apr').text(formatSun(flowAPR))
    $('#marketcap').text(`${formatSunX(marketcap,1)} USD`)
    $('#contractBalance').text(`${formatSun(sumTVL)}`)

    $('#index-aum').text(`${formatSunX(sumTVL, 1)}`)
    $('#index-mcap').text(`${formatSunX(marketcap, 1)}`)
    $('#index-treasury').text(`${formatSunX(treasury, 1)}`)
    $('#index-trunk').text(`${formatSunX(trunkSupply, 1)}`)
...
function refresh(tx) {
    $('#txId').html(`<a href="${tronLinkUrlPrefix}${tx}" target="_blank">${shortId(tx, 5)}</a>`)
    $('#txModal').modal()
    setTimeout(mainLoop)
}
