...
async function isTokenAllowance(token, address, amount) {
    let web3js = getWeb3()
    let allowance = web3js.utils.toBN(await token.methods.allowance(currentAddress, address).call());
...
async function approveTrumpet() {
    let isTRUNK = await isTokenApproved(trunk, trumpet.options.address)
    
    if (!isTRUNK) {
        await approveToken(trunk, trumpet.options.address)
    }

    hideTrumpet()

    showAlert('Approval Processing', 'Please approve tokens to activate Trumpet. Window will close when complete...')
}

async function resetApproveTrumpet() {
    
    await approveToken(trunk, trumpet.options.address)
    
    hideTrumpet()

    showAlert('Approval Processing', 'Please approve tokens to increase approval for Trumpet. Window will close when complete...')
}

async function revokeTrumpet() {
    let isTRUNK = await isTokenApproved(trunk, trumpet.options.address)
    
    if (isTRUNK) {
        await revokeToken(trunk, trumpet.options.address)
    }

    hideTrumpet()

    showAlert('Revoke Processing', 'Please revoke tokens to deactivate Trumpet. Window will close when complete...')
}

async function showTrumpet() {

    let web3js = getWeb3()

    let user = {}

    user.trunk_balance = await trunk.methods.balanceOf(currentAddress).call()
    user.trumpet_balance = await trumpet.methods.balanceOf(currentAddress).call()

    user.approved = await isTokenApproved(trunk, trumpet.options.address)

    let supply = web3js.utils.toBN(await trunk.methods.totalSupply().call())
    user.allowance = await trunk.methods.allowance(currentAddress, trumpet.options.address).call();
    let allowance = web3js.utils.toBN(user.allowance)
    let allowanceStr = (allowance.gte(supply)) ? 'unlimited' : formatSun(allowance)
    let hasBalance = web3js.utils.toBN(user.trumpet_balance).gt(web3js.utils.toBN(0))
    user.approved = await isTokenApproved(trunk, trumpet.options.address) || hasBalance
    ...
    btns = `${(user.approved) ?
        ...
        onclick="mint();">MINT</a>
        <a class="btn btn-outline-semi-light btn-block default mb-0 mr-4 "
        onclick="redeem();">REDEEM</a>
        <a class="btn btn-outline-semi-light btn-block default mt-4 mb-0 mr-4"
        onclick="resetApproveTrumpet();">TRUNK ALLOWANCE  ${approxStr} ${allowanceStr}</a>
            <a class="btn btn-outline-semi-light btn-block default mb-0 mr-4"
            onclick="revokeTrumpet();">REVOKE TRUNK</a>` :
        `<a id="approve-depot" class="btn btn-outline-semi-light btn-block default mb-0 mr-4"
            onclick="approveTrumpet();">APPROVE TRUNK</a>`}`
    tplBody = `
        <div id="topStatsContainer" class="row">
            <div class="container col-6 col-xl-4 col-lg-4 col-md-4  text-center">
                ...
                <img src="/img/logo/trumpet.png" width="100px" class="m-2"/>
                    <p class="text-large mb-2 text-white">${formatSun(user.trumpet_balance)}</p>
                    <p class="text-muted text-small">TRUMPET</p>
                </div>
            </div>
    
            <div class="container col-6 col-xl-4 col-lg-4 col-md-4  text-center">
                <div class="price-top-part">
                    <img src="/img/logo/trunk-logo.png" width="100px" class="m-2"/>
                    <p class="text-large mb-2 text-white">${formatSun(user.trunk_balance)}</p>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Trumpet</h5>
    ...
    $('#showTrumpet').html(tplDialog)
    $('#showTrumpet').modal()
}

async function hideTrumpet() {
    $('#showTrumpet').modal('hide')
}
...
async function mint() {
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Mint TRUMPET</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
    ...
    var calcTokens = async (e) => {
        ...
        try {
            var amount = buyAmountInp.val().trim()
            ...
            if (amount.gt(web3js.utils.toBN(0))){
                let estimate = await trumpet.methods.estimateMinted(amount).call()
                console.log('amount-estimate', estimate)
                trunkEstimate.html(`${approxStr} ${formatSun(estimate)} TRUMPET`)
            } else {
                trunkEstimate.html(`<span class="text-severe">Insufficient amount</span>`)
            }
...
async function redeem() {
    ...
    var balance = (await trumpet.methods.balanceOf(currentAddress).call())

    var tpl = `
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Redeem TRUMPET</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
                    ...
                    <form>
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-9 text-right">
                                    <label class="text-white">${formatSun(balance)}</label> TRUMPET
                                </div>
                            </div>
                            <input type="number" class="form-control" id="stakeAmount"
                                    aria-describedby="buyTrxHelp"
                        ...
                        <p class="text-left">
                            <a class="btn btn-outline-semi-light default mb-0 mr-4"
                                 onclick="redeemTrumpet();">REDEEM</a>
                            <a class="text-underline" href="#" onclick="maxInput('#stakeAmount', '${trumpet.options.address}','${currentAddress}').then(async ()=> await trunkMax()); return false;">Max</a>
                        </p>
                    </form>
            </div>
            <div class="modal-footer text-center">
    ...
    var calcTokens = async (e) => {
        ...
        try {
            ...
            let balance = await trumpet.methods.balanceOf(currentAddress).call()
            amount = resolveInputAndBalance(amount, balance)
            
            if (amount.gt(web3js.utils.toBN(0))){
                let estimate = await trumpet.methods.estimateRedeemed(amount).call()
                console.log('amount-estimate', estimate)
                trunkEstimate.html(`${approxStr} ${formatSun(estimate)} TRUNK`)
            } else {
                trunkEstimate.html(`<span class="text-severe">Insufficient amount</span>`)
            }
...
function hideMintTrumpet() {
    $('#mintTrumpet').modal('hide')
}
...
async function showUserStats() {
    ...
    try {
        ...
        user.trunk_balance = await trunk.methods.balanceOf(currentAddress).call()
        user.trumpet_balance = await trumpet.methods.balanceOf(currentAddress).call()
        user.trumpet_trunk = await trumpet.methods.estimateRedeemed(web3js.utils.toBN(user.trumpet_balance)).call()

        $('#user-trunk').text(formatSun(user.trunk_balance))
        $('#user-trumpet').text(formatSun(user.trumpet_balance))
        $('#user-trumpet-trunk').html(`<br>${approxStr} ${formatSun(user.trumpet_trunk)} TRUNK`)
...
async function showStats() {
    ...
    let trumpetStats =  await trumpet.methods.getInfo().call()
    
    $('#price').text(formatSun(trumpetStats['price']))
    $('#total_users').text(numeral(trumpetStats['users']).format('0,0.000 a'))
    $('#trunk_supply').text(formatSun(trumpetStats['underlyingSupply']))
    $('#total_supply').text(formatSun(trumpetStats['supply']))
    $('#total_txs').text(numeral(trumpetStats['txs']).format('0,0.000 a'))
...
async function mintTrumpet() {
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        if (amount.lt(web3js.utils.toBN(1e18))){
            showAlert('MINT MINIMUM', 'A minimum mint of 1 TRUNK is required')
            return
        }

        let asufficient = await isTokenAllowance(trunk, trumpet.options.address, amount)

        if (!asufficient){
            showAlert('ALLOWANCE INCREASE REQUIRED', 'Click <ALLOWANCE> to update approval limits')
            return
        }

        trumpet.methods.mintWithBacking(amount, currentAddress).send()
            .on('transactionHash', (tx) => showNote('Minting...', tx))
...
async function redeemTrumpet() {
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        if (!amount.gt(web3js.utils.toBN(0))){
            showAlert('REDEMPTION MINIMUM', 'Redemption cannot be zero')
            return
        }

        trumpet.methods.sell(amount).send()
            .on('transactionHash', (tx) => showNote('Redeeming...', tx))
            .on('receipt', (receipt) => showReceipt(receipt.transactionHash))
            .on('error', (e) => txError(e))
    }
