...
var currentAddress
var contractAddress, tokenAddress, proxyAddress
var waiting = 0
...
var balanceFeed = []
let balanceChart, tronLocal, collateral_price, first_block, contract_timestamp, symbol, padding, asset_symbol
...
async function bindUI() {
    ...
    let buyTokens = async (e) => {
        try {
            ...
            buyEstimate.html(`${formatSunX(amount * 0.9, padding)} ${symbol}`)
        } catch (e) {
            console.error(e)
        }
    ...
    let sellTokens = async (e) => {
        try {
            ...
            amount = web3js.utils.toWei(amount + '')
            amount = Math.min(amount, balance)
            sellEstimate.html(`${formatSunX(amount * 0.9, padding)} ${symbol}`)
        } catch (e) {
            console.error(e)
        }
...
async function showUserStats() {
    let [stats, balance, divs, estimate, totalSupply,dividendBalance,collateralBalance] =
        await Promise.all(
            [
                ...
                stack.methods.totalSupply().call(),
                stack.methods.dividendBalance_().call(),
                collateral.methods.balanceOf(currentAddress).call()
            ]
        )
    ...
    $('.user-balance').text(formatSunX(balance, padding))
    $('.collateral-balance').text(formatSunX(collateralBalance, padding))
   

    $('#user-vault').text(formatSunX(divs, padding))
    //$('#user-vault-usdt').html(divs > 0 ? `${approxStr} ${formatSun(divs)} USDT` : '')

    $('#user-estimate').html((apy && balance/1e18 > 0.0001) ? `&#8776; ${numeral(apy).format('0.000')} % APR` : '')
...
async function sweep() {
    ...
    if ((await stack.methods.elephantReserve_().call() == 0)) {
        showAlert('NO REWARDS!!!', 'Slow down there buddy, we need to accumulate more rewards before buying back ELEPHANT!')
        return
    }

    stack.methods.sweep().send()
        .on('transactionHash', (tx) => refresh(tx))
        .on('error', (e) => txError(e))
...
async function loadTransactions() {
    ...
    try {
        await stack.getPastEvents('allEvents', {
            ...
            _.forEach(results, async result => {
                ...
                */
                if (_.isUndefined(window.isElephantStack)){
                    ...
                    if (result.event == 'onTokenPurchase' || result.event == 'onTokenSell') {
                        event.time = moment.unix(result.returnValues.timestamp).fromNow()
                        event.timestamp = result.returnValues.timestamp
                        event.txUrl = formatTxUrl(result.transactionHash)
                        event.player = result.returnValues.customerAddress
            ...
            buyEvents = _.uniqBy(buyEvents, 'txUrl')
...
async function loadLeaderBoard() {
    ...
    let block = await web3.eth.getBlockNumber() - 5
    let fromBlock =  first_block//Math.max(first_block, block - 17280 * 30) //block - 4500
    ...
    try {
        ...
        results = await stack.getPastEvents('onLeaderBoard', {
            fromBlock: fromBlock, toBlock: Math.min(fromBlock + 4000, block) 
        }) 
    ...
    while (fromBlock < block ) {
        ...
        console.log('block range', fromBlock, toBlock, "block height", block)    
         results = await stack.getPastEvents('onLeaderBoard', {
            fromBlock: fromBlock, toBlock: Math.min(fromBlock + 4000, block) 
        }) 
