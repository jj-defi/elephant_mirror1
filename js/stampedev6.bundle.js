...
async function main() {
    ...
    if (!(window.ethDefaultAddress)) {
        ...
        await showFuturesAsk()

    }

}

async function showFuturesAsk() {
    let futureUser = await futures.methods.getUser(currentAddress).call()
    if (futureUser['exists']  == false){
        showAlert('JOIN FUTURES TODAY', `Futures works just like Stampede with a fixed 0.5% paid out daily down to the second.  
        You don't have to wait for TRUNK peg to grow passive income. Futures is key to the recovery and is a win win for all involved.
          Any level of participation will make a huge difference for your ELEPHANT and TRUNK positions. Get started today. Thank You! DYOR-NFA`)
    } else {
        showAlert('BOSS STAMPEDE USER', `Thanks for joining Futures! Deposits should be focused on Futures while we are in recovery! 
        TRUNK will remain volatile as BERTHA grows to full strength. DYOR-NFA`)
    }
}
...
async function approveStampede() {
    let isTRUNK = await isTokenApproved(trunk, stampede.options.address)
    
    if (!isTRUNK) {
        await approveToken(trunk, stampede.options.address)
    }

    hideStampede()

    showAlert('Approval Processing', 'Please approve tokens to activate Stampede. Window will close when complete...')
}

async function revokeStampede() {
    let isTRUNK = await isTokenApproved(trunk, stampede.options.address)
    
    if (isTRUNK) {
        await revokeToken(trunk, stampede.options.address)
    }

    hideStampede()

    showAlert('Revoke Processing', 'Please revoke tokens to deactivate Stampede. Window will close when complete...')
}

async function showStampede() {

    let user = {}
    let stampedeUser = await stampede.methods.getUser(currentAddress).call()
    let eligibleUser = await stampede.methods.eligibleForMigration(currentAddress).call()

    if (eligibleUser['eligible']){
        user.eligible = true
        user.deposits = eligibleUser['deposits']
        user.claimed = eligibleUser['payouts']
        user.last_time = (eligibleUser['last_time'] == 0) ? 'Never' : moment().diff(moment.unix(eligibleUser['last_time']), "days")
        user.last_time = (user.last_time == '0') ? 'Recent' : `${user.last_time} days ago` 
    } else {
        user.eligible = false
        user.exists = stampedeUser['exists']
        user.deposits = stampedeUser['deposits']
        user.current_balance = stampedeUser['current_balance']
        user.claimed = stampedeUser['payouts']
        user.rewards = stampedeUser['rewards']

        user.last_time = (stampedeUser['last_time'] == 0) ? 'Never' : moment().diff(moment.unix(stampedeUser['last_time']), "days")
        user.last_time = (user.last_time == '0') ? 'Recent' : `${user.last_time} days ago` 
    
        user.approved = await isTokenApproved(trunk, stampede.options.address)

        let available  = await stampede.methods.available(currentAddress).call()
        user.divs = available['_adjustedAmount']
    }
    ...
    if (user.eligible){
        btns = `<a class="btn btn-outline-semi-light btn-block default mb-0 mr-4"
        onclick="migrate();">MIGRATE</a>`
        tplBody = `
            <div id="topStatsContainer" class="row">
                ...
                <p class="col-12 white mb-3 text-center">
                    The last interaction was - ${user.last_time}
                </p>
        ...
        btns = `${(user.approved) ?
            ...
            onclick="depositStampede();">DEPOSIT</a>
            <a class="btn btn-outline-semi-light btn-block default mb-0 mr-4 "
            onclick="airdropStampede();">AIRDROP</a>
            <a class="btn btn-outline-semi-light btn-block default mb-4 mr-4"
                onclick="claimStampede();">CLAIM</a>
                <a class="btn btn-outline-semi-light btn-block default mb-0 mr-4"
                onclick="revokeStampede();">REVOKE</a>` :
            `<a id="approve-depot" class="btn btn-outline-semi-light btn-block default mb-0 mr-4"
                onclick="approveStampede();">APPROVE TRUNK</a>`}`
        tplBody = `
            <div id="topStatsContainer" class="row">
                <!-- <h4 class="col-12 text-center pb-2 mb-2"><a href="/trunk.html" target="_blank" class="mb-2">Get TRUNK</a></h4> -->
                <div class="container col-6 col-xl-4 col-lg-4 col-md-4  text-center">
                    <div class="price-top-part">
                        ...
                        <p class="text-large mb-2 text-white">${formatSun(user.divs,3)}</p>
                        <p class="text-muted text-small">TRUNK</p>
                    </div>
                </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Stampede</h5>
    ...
    $('#showStampede').html(tplDialog)
    $('#showStampede').modal()
}

async function hideStampede() {
    $('#showStampede').modal('hide')
}

async function depositStampede() {
    let web3js = getWeb3();
    hideStampede()

    var balance = (await trunk.methods.balanceOf(currentAddress).call())

    var tpl = `
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Deposit TRUNK</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
                    ...
                    <form>
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-9 text-right">
                                    <label class="text-white">${formatSun(balance)}</label> TRUNK
                                </div>
                            </div>
                        ...
                        <p class="text-left">
                            <a class="btn btn-outline-semi-light default mb-0 mr-4"
                                 onclick="freezeStampede();">DEPOSIT</a>
                            <a class="text-underline" href="#" onclick="maxInput('#stakeAmount', '${trunk.options.address}','${currentAddress}').then(async ()=> await trunkMax()); return false;">Max</a>
                        </p>
                    </form>
            </div>
            <div class="modal-footer text-center">
    ...
    $('#depositStampede').modal()
}

function hideDepositStampede() {
    $('#depositStampede').modal('hide')
}

async function airdropStampede() {
    let web3js = getWeb3();
    hideStampede()
            ...
            <div class="modal-body">
                    <form>
                        <div class="form-group">
                            ...
                            <input class="form-control" id="sponsor-recipient" aria-describedby="transTrxHelp" placeholder="Address">
                        </div>
                        <div class="form-group">
                            <div class="row">
                                <div class="col-3 text-left">
                        ...
                        <p class="text-left">
                            <a class="btn btn-outline-semi-light default mb-0 mr-4"
                                 onclick="sponsorStampede();">AIRDROP</a>
                            <a class="text-underline" href="#" onclick="maxInput('#stakeAmount', '${trunk.options.address}','${currentAddress}').then(async ()=> await trunkMax()); return false;">Max</a>
                        </p>
                    </form>
            </div>
...
async function showUserStats() {
    ...
    if (eligibleUser['eligible']){
        ...
        user.claimed = eligibleUser['payouts']
        user.rewards = 0*/
        $('#topStatsContainer').html('&nbsp')
        $('#show-stampede-btn').text('MIGRATE TO STAMPEDE v6')
    } else {

        user.exists = stampedeUser['exists']
        user.deposits = stampedeUser['deposits']
        user.deposit_base = stampedeUser['deposit_base']
        user.current_balance = stampedeUser['current_balance']
        user.claimed = stampedeUser['payouts']
        user.rewards = stampedeUser['rewards']
        let available = await stampede.methods.available(currentAddress).call()
        ...
        $('#user-divs').text(formatSun(user.divs))
        $('#user-balance').text(formatSun(user.current_balance))
        $('#user-claimed').text(formatSun(user.claimed))
        $('#user-rewards').text(formatSun(user.rewards))
    }
...
async function showStats() {
    ...
    let stampedeStats =  await stampede.methods.getInfo().call()

    //let bufferBalance = await trunk.methods.balanceOf(trunkBuffer.options.address).call()

    let price = await getElephantPrice(1e9)
    let elephantTreasuryBalance = await elephant.methods.balanceOf(elephantTreasury.options.address).call()
    let elephantTreasuryBalanceUSD = (elephantTreasuryBalance > 0) ? (price * elephantTreasuryBalance) / 1e9 : 0
    let trunkTreasuryBalance = await trunk.methods.balanceOf(trunkTreasury.options.address).call()
    let busdTreasuryBalance = await busd.methods.balanceOf(busdTreasury.options.address).call()
    let annualApr = await stampede.methods.scaleByPeg(web3js.utils.toBN(182.5e18)).call()
    let dailyLiabilities = parseInt(await stampede.methods.scaleByPeg(web3js.utils.toBN(stampedeStats['current_balance'])).call())

    $('#annual-apr').html(`${approxStr} ${formatSun(annualApr)} % APR`) 
    $('#stampede_total_users').text(numeral(stampedeStats['total_users']).format('0,0.000 a'))
    $('#stampede_total_deposited').text(formatSun(stampedeStats['total_deposited']))
    $('#stampede_current_balance').text(formatSun(stampedeStats['current_balance']))
    $('#stampede_total_claimed').text(formatSun(stampedeStats['total_claimed']))
    $('#stampede_total_rewards').text(formatSun(stampedeStats['total_rewards']))
    $('#stampede_total_txs').text(numeral(stampedeStats['total_txs']).format('0,0.000 a'))
    $('#trunk-daily').text(formatSun( 0.005 *  dailyLiabilities))
    //$('#trunk-buffer').text(formatSun(bufferBalance))
...
async function freezeStampede() {
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        if (amount.lt(web3js.utils.toBN(200e18))){
            showAlert('DEPOSIT MINIMUM', 'A minimum deposit of 200 TRUNK is required')
            return
        }

        stampede.methods.deposit(amount).send()
...
async function claimStampede() {

    hideStampede()
    let available = await stampede.methods.available(currentAddress).call()
    if (!(parseInt(available['_adjustedAmount']))) {
        showAlert('NO DIVS!!!', 'Slow down there buddy, you need to have some divs first!')
        return
    }
    //stampede.methods.claim().estimateGas(console.log)
    stampede.methods.claim().send()
...
async function sponsorStampede() {
    let web3js = getWeb3()
    hideDepositStampede()
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        if (!web3js.utils.isAddress(address) || address == currentAddress) {
            ...
            if (amount.lt(web3js.utils.toBN(1e18))){
                showAlert('AIRDROP MINIMUM', 'A minimum deposit of 1 TRUNK is required')
                return
            }

            stampede.methods.sponsor(address, amount).send()
                .on('transactionHash', (tx) => showNote('Airdropping...', tx))
                .on('receipt', (receipt) => showReceipt(receipt.transactionHash))
                .on('error', (e) => txError(e))
        }   
    }
...
async function migrate() {
    ...
    //stampede.methods.migrate().estimateGas(console.log)
    stampede.methods.migrate().send()
        .on('transactionHash', (tx) => showNote('Migrating...', tx))
        .on('receipt', async (receipt) => {
            location.reload()
        })
        .on('error', (e) => txError(e))
    showAlert('JOIN FUTURES TODAY', 'The best way pump ELEPHANT and TRUNK while earning 0.5% a day is with Futures. $33K deposited over 3 years becomes over $1.3M. Start your journey today!')
    return false
