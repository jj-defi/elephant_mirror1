...
async function approveFutures() {
    let isBUSD = await isTokenApproved(busd, futures.options.address)
    
    if (!isBUSD) {
        await approveToken(busd, futures.options.address)
    }

    hideFutures()

    showAlert('Approval Processing', 'Please approve tokens to activate Futures. Window will close when complete...')
}

async function revokeFutures() {
    let isBUSD = await isTokenApproved(busd, futures.options.address)
    
    if (isBUSD) {
        await revokeToken(busd, futures.options.address)
    }

    hideFutures()

    showAlert('Revoke Processing', 'Please revoke tokens to deactivate Futures. Window will close when complete...')
}
...
async function showFutures() {
    ...
    user.exists = futureUser['exists']
    user.deposits = futureUser['deposits']
    user.current_balance = futureUser['current_balance']
    user.claimed = futureUser['payouts']
    user.rewards = futureUser['rewards']

    //user.last_time = (futureUser['last_time'] == 0) ? 'Never' : moment.unix(futureUser['last_time']).fromNow()
    user.last_time = (futureUser['last_time'] == 0) ? 'Never' : moment().diff(moment.unix(futureUser['last_time']), "days")
    user.last_time = (user.last_time == '0') ? 'Recent' : `${user.last_time} days ago` 
   
    user.approved = await isTokenApproved(busd, futures.options.address)

    let available  = await futures.methods.available(currentAddress).call()
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Futures</h5>
    ...
    $('#showFutures').html(tplDialog)
    $('#showFutures').modal()
}

async function hideFutures() {
    $('#showFutures').modal('hide')
}

async function depositFutures() {
    let web3js = getWeb3();
    hideFutures()
                    ...
                    <form>
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-9 text-right">
                                    <label class="text-white">${formatSun(balance)}</label> BUSD
                                </div>
                            </div>
                        ...
                        <p class="text-left">
                            <a class="btn btn-outline-semi-light default mb-0 mr-4"
                                 onclick="freezeFutures();">DEPOSIT</a>
    ...
    var calcTokens = async (e) => {
        ...
        try {
            ...
            trunkEstimate.html(`${approxStr} ${formatSun(amount)} BUSD`)
            } catch (error) {
                trunkEstimate.html(`&nbsp;`)
        }
    ...
    $('#depositFutures').modal()
}

function hideDepositFutures() {
    $('#depositFutures').modal('hide')
}
...
async function showUserStats() {
    ...
    user.exists = futureUser['exists']
    user.deposits = futureUser['deposits']
    user.deposit_base = futureUser['deposit_base']
    user.current_balance = futureUser['current_balance']
    user.claimed = futureUser['payouts']
    user.rewards = futureUser['rewards']
    let available = await futures.methods.available(currentAddress).call()
...
async function showStats() {
    ...
    let depotStats =  await futures.methods.getInfo().call()

    let bufferBalance = await busd.methods.balanceOf(busdBuffer.options.address).call()
    ...
    $('#futures_total_users').text(numeral(depotStats['total_users']).format('0,0.000 a'))
    $('#futures_total_deposited').text(formatSun(depotStats['total_deposited']))
    $('#futures_current_balance').text(formatSun(depotStats['current_balance']))
    $('#futures_total_claimed').text(formatSun(depotStats['total_claimed']))
    $('#futures_total_rewards').text(formatSun(depotStats['total_rewards']))
    $('#futures_total_txs').text(numeral(depotStats['total_txs']).format('0,0.000 a'))
    $('#busd-daily').text(formatSun( 0.005 *  depotStats['current_balance']))
    $('#busd-buffer').text(formatSun(bufferBalance))
...
async function freezeFutures() {
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        if (amount.lt(web3js.utils.toBN(200e18))){
            showAlert('DEPOSIT MINIMUM', 'A minimum deposit of 200 BUSD is required')
            return
        }

        futures.methods.deposit(amount).send()
...
async function claimFutures() {
    ...
    //futures.methods.claim().estimateGas(console.log)
    futures.methods.claim().send()
